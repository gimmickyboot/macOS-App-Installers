...
bundleName="Azure Developer CLI"
appName="${bundleName}"
installedVers=$("${appInstallPath}"/"${bundleName}" version | /usr/bin.awk '{print $3}')
...
downloadURL="https://github.com$(/usr/bin/curl -sL "$(printf '%s' "${latestReleaseURL}" | /usr/bin/sed 's/tag/expanded_assets/')" | /usr/bin/grep "azd-darwin-${archType}" | /usr/bin/head -n 1 | /usr/bin/xmllint --html --xpath 'string(//a/@href)' -)"
FILE=${downloadURL##*/}
...
if /usr/bin/curl --retry 3 --retry-delay 0 --retry-all-errors -sL "${downloadURL}" -o /tmp/"${FILE}"; then
  ...
  /bin/rm "${appInstallPath}"/"${bundleName}"
  /usr/bin/unzip /tmp/"${FILE}" -d /tmp
  /bin/mv /tmp/azd-darwin-"${archType}" "${appInstallPath}"/"${bundleName}"
  /bin/chmod a+x /usr/local/bin/"${bundleName}"
  printf '%s\n' "Successfully installed ${FILE}"
  /bin/rm -rf /tmp/${bundleName}*
fi
