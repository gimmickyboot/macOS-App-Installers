...
bundleName=".Net SDK 11 Preview"
appName="${bundleName}"
installedVers=$(/usr/bin/find "${appInstallPath}" -mindepth 1 -maxdepth 1 -type d -name '11*' -exec basename {} \; | /usr/bin/sort -V | /usr/bin/tail -n 1 | rev | /usr/bin/cut -d "." -f 3- - | rev)
