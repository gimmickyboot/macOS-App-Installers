...
currentVers=$(/usr/bin/curl -s "https://www.blender.org/download/" | /usr/bin/grep dmg | /usr/bin/head -n 1 | /usr/bin/cut -d \" -f 2 - | /usr/bin/sed -n 's/.*blender-\([0-9.]*\)-macos.*/\1/p')
majVers=$(/bin/echo "${currentVers}" | /usr/bin/cut -d . -f 1-2 -)
case $(uname -m) in
  arm64)
    archType="arm64"
    ;;
...
downloadURL="https://download.blender.org/release/Blender${majVers}/blender-${currentVers}-macos-${archType}.dmg"
FILE=${downloadURL##*/}
